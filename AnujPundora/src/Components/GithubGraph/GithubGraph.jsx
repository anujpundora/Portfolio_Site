...
function GithubGraph() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!weeks) {
          console.error("Unexpected GitHub data structure:", data);
          return;
        }
        ...
        // Create all dates for the current month
  ...
  return (
    <div className="flex flex-col items-center gap-10 md:mt-0 mt-30 md:pb-20 justify-center w-full">
      <h1 id="heading" className="flex flex-wrap ml-5 md:ml-0  gap-2 text-5xl">
        GITHUB CONTRIBUTION FOR THIS
        <span className="font-bold text-[#FFB606]">MONTH</span>
      </h1>

      <div className="rounded-xl bg-[#1A1A1A] hover:shadow-[0_0_20px_#fff2b8] w-4/5 md:w-2/5 font-thin">
        <h1 className="m-5 mb-2.5 text-2xl">{monthName}</h1>
        ...
        <div className="grid grid-rows-7 grid-flow-col w-full gap-x-0 gap-y-2 p-10 pb-5 pt-0 pr-0">
          {days.map((day) => (
            <div key={day.date} className="relative group">
              <div
                style={{
                  backgroundColor: getGoldColor(day.contributionCount),
                }}
        ...
        <div className="flex justify-between px-2.5 pb-2.5">
          <h1>This month contributions: {totalContributions}</h1>
          <div className="flex gap-2">
            <p>Less</p>
            <div className="h-5 w-5 rounded-sm bg-[#d3d3d3]" />
            <div className="h-5 w-5 rounded-sm bg-[#fff2b8]" />
            <div className="h-5 w-5 rounded-sm bg-[#ffe066]" />
            <div className="h-5 w-5 rounded-sm bg-[#ffd700]" />
            <p>More</p>
          </div>
        </div>
      </div>
